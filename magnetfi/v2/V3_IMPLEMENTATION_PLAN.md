...
## Phase 3 — Folks adapter + integration  ✅ 3.1 BUILT / ✅ 3.2 VALIDATED ON TESTNET

> **3.1 done.** Folks v2 USDC-pool interface reverse-engineered + live-verified; `FolksAdapter` built to `FOLKS_ADAPTER.md`, compiles clean, same interface selectors as the mock.
> **3.2 done.** `tests/test_folks_adapter_testnet.py` ran the full **deposit → recoverable read → withdraw** cycle against the **real Folks v2 testnet** pool (`147170678`): 0.5 USDC → 418,636 fUSDC; `recoverable_value` = 499,999 µUSDC matching `fUSDC × live index / 1e14` exactly; withdraw returned 499,999. Fee budget / rounding / receiver-routing all cleared. **Learning:** Folks ops need extra foreign-reference slots — pad groups with filler app calls (Phase 4).
> **Full-stack done.** `tests/test_psm_folks_testnet.py` runs the **real PSMv3** (a short-timelock build generated from the real source — only the constant patched, no logic drift) → **real FolksAdapter** → **live Folks**: `strategy_deploy` (0.3 USDC, PSMv3's invariant post-check live-reads the adapter through Folks) → `strategy_recall` (full recovery, zero deficit, adapter emptied) → `remove_adapter` → USDC round-tripped to the wallet (net 1 µUSDC dust). Composition proven end-to-end. **Remaining before mainnet:** dedicated FolksAdapter audit + a bounded mainnet canary.
