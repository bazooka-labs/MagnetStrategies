...
- [ ] **Bot redundancy & alerting (AUD-004).** Run redundant bot instances and uptime/staleness alerting so the oracle doesn't go stale on a single-host failure. (Oracle staleness fails safe — blocks borrows/liquidations — but is still an availability hit.)
  - **Staleness alert — design (shelved 2026-08-25, discussed but not built):**
    - **Watch the chain, not the bot.** Monitor on-chain `lp_ts_<poolId>` in the oracle (app `3644230020`); alert when `now − lp_ts > threshold`. This catches *every* failure mode incl. the one we hit — bot alive but posts rejected (the `pc=512` anchor-band rejection, see ADMIN.md). A "process up?" check would have missed it. Read-only (public algod), no secrets.
    - **Run it independent of the home console** (the bot's host) so a machine/network outage can't silence it. Preferred: a **Vercel Cron** hitting a small in-repo API route; alternatives: separate cron on the console (shares fate with bot), or an external uptime service.
    - **Threshold:** warn early (~15 min = 3 missed ~5-min cycles) — before the 30-min freshness window actually blocks borrows.
    - **Channel:** Discord webhook is the simplest (already have a Discord); optionally Telegram/email/SMS for a critical tier. Debounce: one alert + periodic reminders + a "recovered" message.
    - **Make it proactive too:** warn when a pool's price is within ~5% of its anchor-band edge (would have caught the U/tALGO outage a day early) and when the bot wallet ALGO drops below ~2.
    - Decisions still open: location, channel(s), reactive-only vs proactive.
