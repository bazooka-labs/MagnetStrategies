...
**Why no AssetTransfer assertion:** mUSD physically reaches PSM through two different paths depending on which vault method calls receive_musd. In `settle_health_liquidation()`, mUSD arrives via an outer group AssetTransfer (admin → PSM) that the vault verifies at its level. In `pay_interest()` principal repayment, mUSD arrives via a vault-issued inner AssetTransfer (vault → PSM) submitted as a sibling to the receive_musd call. Algorand's Gtxn always refers to the OUTER transaction group, so a Gtxn assertion in receive_musd would catch case 1 but always fail in case 2. Security relies on the vault app address check (only the vault can call this) and the physical token flow (mUSD must arrive at PSM for PSM's ASA balance to increase, which is what any caller reading circulating supply observes).
...
- **`harvest(adapter)`** — sweeps only the **yield** (`recoverable_value − deployed_principal`, if positive) to **treasury**. Separate from `recall`, so topping up the buffer never forces a yield sweep. **Trust-minimized on the buffer (F-1, hardened per code-audit H-2):** harvest routes only the **on-chain USDC actually received this tx**, capped at the computed yield — measured by the PSM's *own* USDC balance delta, **never the adapter's reported return value**. So an adapter can never pull the PSM's pre-existing buffer to treasury (only funds it itself holds), and any over-return just stays as backing. It still `asserts recoverable_after ≥ deployed_principal` after the sweep as defense-in-depth.
  - **Accepted residual venue risk (code-audit H-1):** both the sweep bound and that post-sweep assert read the adapter's *own* `recoverable_value()`. A whitelisted adapter that lies self-consistently could route **its own deployed principal** (bounded — never the buffer or other venues) to treasury and leave phantom backing, revealed as a deficit on the next `recall`. This is the *same* irreducible venue trust as `min()`: **harvest safety requires `recoverable_value()` to be a non-manipulable on-chain read** — the **#1 gate of every adapter's dedicated audit**. The Folks adapter reads `fUSDC_balance × index` from real on-chain state it cannot fake (sending principal out drops its real balance → the self-check fires), so it is not exploitable in practice. Harvest is additionally frozen while paused, in deficit, or while **any** adapter is impaired.
  - **Blast-radius note (audit L-B):** the "bounded to its own deployed principal" property also applies to **`withdraw_usdc`** — its headroom check reads `_total_backing()`, so a lying adapter's inflated `recoverable` (still capped at that adapter's principal by `min()`) can raise how much real buffer USDC an admin could withdraw. It remains bounded to that adapter's principal and requires an admin action (funds route to `admin`), so it does not breach the "never the buffer or other venues" guarantee — but the harvest **and** `withdraw_usdc` paths both inherit the `recoverable_value()`-honesty dependency. A 2026-08-14 adversarial audit confirmed all three non-negotiables hold and the residual is bounded exactly as stated.
